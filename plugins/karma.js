...
function updateKarma(message, groups, options, proceedure) {
  ...
  storage
    .init({ dir: config.karma.karma_log })
      ...
      if (proceedure === 'increment') {
        ...
        if (!currentKarma) {
          message.reply_thread(
            `<@${groups.user}> has no Karma to deduct from!`
          );
        } else {
    ...
    .then((requesterKarma) => {
      ...
      if (proceedure === 'decrement') {
        ...
        message.reply_thread(
          `2 karma points have been take from you as well <@${message.user}>. Your new balance is: ${totalKarma}`
        );
      }
    });
}
...
module.exports = {
  name: 'karma',
  help:
    "Give or deducte someone's karma points. Remember that if you deduct, 2 karma points will be taken from you as a penalty!",
  verbose,
  events
};
