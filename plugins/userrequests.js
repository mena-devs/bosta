...
const META = {
  name: 'userrequests',
  short:
    'Request an invite for a user -- Do not forget the ( ) they are necessary!',
  examples: ['@bosta invite (Full Name) (Email) (Occupation) (Company)']
};
...
its status! :wink:`;
  web.chat.postMessage(userID, msg, { as_user: true }, (error) => {
    if (error) {
      winston.error(
        `${META.name} Could not respond to invitation requesting user:`,
        error
      );
    } else {
  ...
  rp(options)
    .then(function (response) {
      if (response.statusCode == 201) {
        informUserRequestApproved(
          web,
          invitationRequestObj.invitee_name,
          invitationRequestObj.slack_uid
        );
      } else if (response.statusCode == 422) {
        // TODO -- Handle duplicate errors in a separate manner than
        // rejected requests by admins
        informUserRequestDenied(
          web,
          invitationRequestObj.invitee_name,
          invitationRequestObj.slack_uid
        );
...
function register(bot, rtm, web, config, secret) {
  rtm.on(RTM_EVENTS.MESSAGE, (message) => {
    if (message.text) {
      ...
      const [, target, fullname, email, occupation, company] =
      ...
      if (target === bot.self.id) {
        if (
          ...
          const postChannel =
            config.plugins.userrequests.invitation_request_channel;
          const attachment = {
            as_user: true,
            attachments: [
              {
          ...
          web.chat.postMessage(postChannel, '', attachment, (error) => {
            if (error) {
              winston.error(
                `Could not post invitation request to ${postChannel}`,
                error
              );
            } else {
    ...
    if (message.reaction == 'white_check_mark') {
      web.groups
        ...
        .then((response) => {
          ...
          const cleanRequesterId = messageFields[0].value
            .slice(1, -1)
            .split('@')[1];
        ...
        .catch((error) => {
          winston.error(
            `${META.name} - Processing Invitation Error - : ${error}`
          );
        });
