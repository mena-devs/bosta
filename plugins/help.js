...
const { Blocks, Button, Divider, Markdown, Section } = require('../blocks.js');
...
const buildBlocks = () => {
  const helps = Object.values(plugins).map((plugin) =>
    Section(
...
const buildHelpBlocks = (plugin) => {
  ...
  return Blocks(
    Section(
      Markdown(`*${selected.name}:* ${selected.help}\n${selected.verbose}`)
    ),
    Divider()
  );
};
